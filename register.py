...
def get_credentials(region):
    region = region.upper()
    if region == "IND":
        return "4402591678", "Stream-2FRAMXQIY-X"
    elif region in ["NA", "BR", "SAC", "US"]:
        return "4446787324", "Stream-IIHJVWIW3-X"
    else:
        return "4446790683", "Stream-A7VXGZK5E-X"
...
def main():
    ...
    if not jwt_info or 'token' not in jwt_info or not jwt_info.get('serverUrl'):
    ...
    result['credits'] = {
        ...
        "api_channel": "t.me/dangerfreeapis"
    }
